...
const CartItem = ({ onContinueShopping, onCheckoutShopping }) => {
  ...
  const handleContinueShopping = (e) => {
      onContinueShopping({});  // passing an empty object as a dummy event
  };
 ...
 const handleCheckoutShopping =(e) =>{
  ...
  const handleDecrement = (item) => {
    if(item.quantity > 1 ){
      dispatch(updateQuantity({name: item.name, quantity: item.quantity -1}));// if quantity is more than 1, remove the item
    }
  };
